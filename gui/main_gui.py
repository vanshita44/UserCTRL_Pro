...
class UserCTRLApp(tk.Tk):
    ...
    def setup_user_tab(self):
        ...
        self.user_listbox.bind("<Double-1>", self.on_user_select)
...
class UserDialog(tk.Toplevel):
    ...
    def setup_add_user(self):
        ...
        self.role.current(1)  # Default to student
    ...
    def setup_modify_user(self, preselected_username):
        ...
        # New Shell
        ttk.Label(form_frame, text="New Shell:").grid(row=2, column=0, padx=5, pady=5, sticky="e")
        self.shell = ttk.Combobox(form_frame, values=["/bin/bash", "/bin/sh", "/bin/zsh"], width=28)
        self.shell.grid(row=2, column=1, padx=5, pady=5)
        
        # New Home Directory
        ttk.Label(form_frame, text="New Home Directory:").grid(row=3, column=0, padx=5, pady=5, sticky="e")
        self.home_dir = ttk.Entry(form_frame, width=30)
        self.home_dir.grid(row=3, column=1, padx=5, pady=5)
        
        # Move Home Contents
        self.move_home = tk.BooleanVar(value=True)
        ttk.Checkbutton(form_frame, text="Move home directory contents", variable=self.move_home).grid(row=4, column=1, padx=5, pady=5, sticky="w")
        
        # Groups to Add
        ttk.Label(form_frame, text="Add to Groups:").grid(row=5, column=0, padx=5, pady=5, sticky="e")
        self.groups = ttk.Entry(form_frame, width=30)
        self.groups.grid(row=5, column=1, padx=5, pady=5)
        ttk.Label(form_frame, text="(comma-separated)").grid(row=5, column=1, padx=(250, 5), pady=5, sticky="w")
    ...
    def execute_modify_user(self):
        ...
        if not any([new_username, shell, home_dir, groups]):
